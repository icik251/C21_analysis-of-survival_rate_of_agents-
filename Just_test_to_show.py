...
#Load assets
RED_AGENT = pygame.image.load(os.path.join("C:\python_projects\dmas_pull\C21_analysis-of-survival_rate_of_agents-\Pygame_Simulation\dassets","pixel_ship_red_small.png"))
#Lasers
RED_LASER = pygame.image.load(os.path.join(
	"C:\python_projects\dmas_pull\C21_analysis-of-survival_rate_of_agents-\Pygame_Simulation\dassets","pixel_laser_red.png"))
#Backgrond
BG = pygame.transform.scale(pygame.image.load(os.path.join("C:\python_projects\dmas_pull\C21_analysis-of-survival_rate_of_agents-\Pygame_Simulation\dassets","background-black.png")),(WIDTH,HEIGHT))


#This full class defines agents, each with its own attributes
class Agent:         
	def __init__(self, x, y, health=3, cover = False, covered_another = False , battery = 1):
	...
	def draw(self, window):                                       	  #To show/draw the agent on the screen
		...
		self.battery_healthbar(window)				  #Also draw the healthbar for each agent
	...
	def battery_healthbar(self, window):
		pygame.draw.rect(window, (255,0,0), (self.x, self.y + self.get_height() + 20  , self.ship_img.get_width(), 10))
		pygame.draw.rect(window, (0,255,0), (self.x, self.y + self.get_height() + 20 , self.ship_img.get_width() * (self.battery/self.max_battery), 10))


#This full class defines the bullets/lasers
class Bullet:
	def __init__(self, x, y, health=3, battery =3):
		self.x = x
		self.y = y
		self.health = health
	...
	while run:
		...
		if len(agents) == 0 and no_of_waves>0:						#Logic states, If there is no agent on the screen and the required number of waves have not gone already, send in the next wave
			for i in range(wave_length):						#initialising to run the loop "length" number of times to create the required number of agents
				...
				xpos += 100							#increasing Xpos value for next agent
		...
		for agent in agents[:]:
			for tnega in agents:
				if agent != tnega:
					if abs(agent.x - tnega.x)<cover_radius and agent.health == 1 and agent.cover == False and tnega.health != 1:
						cover(agent,tnega)
