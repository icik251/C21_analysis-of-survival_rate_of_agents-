    ...
    def save_results(self):
        ...
        if not self.is_exp_number_provided():
            ...
            text_file = open(
                os.path.join(
                    self.dir_name,
                    "exp_{}_random_seed_{}.txt".format(
                        self.exp_number,
                        self.config_dict['random_seed'])),
                "w")
            text_file.write(
            ...
            text_file.write(
                "Agent covering (Boolean): %s\n" %
                self.config_dict['to_cover'])
            text_file.write(
                "Cost Function Value: %s\n" %
                self.config_dict['cost_value'])
            text_file.write(
                "Config Dictionary for Graph: %s\n" %
                self.agents_to_wave)
            text_file.write(
                "Config Dictionary for Comp Time: %s\n" %
